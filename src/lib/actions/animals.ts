...
export async function getAnimals(farmIdOrUserId: string): Promise<Animal[]> {
    try {
        ...
        return animals.map(a => ({
            ...
            species: a.species,
            breed: a.breed,
            dob: a.dob ? (a.dob instanceof Date ? a.dob.toISOString().split('T')[0] : String(a.dob)) : '',
            gender: a.gender,
            status: a.status,
            purpose: a.purpose,
            farmId: a.farm_id,
            lastPregnancy: a.last_pregnancy ? (a.last_pregnancy instanceof Date ? a.last_pregnancy.toISOString().split('T')[0] : String(a.last_pregnancy)) : '',
