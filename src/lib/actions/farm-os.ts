    ...
    try {
        ...
        await db`
            INSERT INTO health_logs (
                ...
                ${id}, ${data.animalId}, ${data.eventType}, ${data.description || null}, 
                ${data.date}, ${data.cost || null}, ${data.medication || null}, ${data.notes || null}
            )
        `;
    ...
    try {
        ...
        await db`
            INSERT INTO production_logs (
                ...
                ${id}, ${data.animalId}, ${data.date}, ${data.metricType}, ${data.value}, ${data.unit}
            )
        `;
        revalidatePath('/[locale]/animal/[animalId]', 'page');
        return { success: true, id };
    ...
    try {
        ...
        await db`
            INSERT INTO breeding_records (
                ...
                ${id}, ${data.animalId}, ${data.sireId || null}, ${data.eventType}, 
                ${data.date}, ${data.result || null}, ${data.expectedDueDate || null}
            )
        `;
        revalidatePath('/[locale]/animal/[animalId]', 'page');
        return { success: true, id };
        ...
        if (data.id) {
            ...
            await db`
                INSERT INTO feed_inventory (
                    ...
                    ${id}, ${data.farmId}, ${data.feedType}, ${data.currentQuantity}, 
                    ${data.unit}, ${data.reorderLevel || null}
                )
            `;
            return { success: true, id };
        }
